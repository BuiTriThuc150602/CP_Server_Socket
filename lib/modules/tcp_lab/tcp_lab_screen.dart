...
class _TcpLabScreenState extends State<TcpLabScreen> {
  ...
  void initState() {
    ...
    _subscriptions.addAll([
      ...
      _client.incoming.listen((text) => _addConsole(SocketConsoleEntry(timestamp: DateTime.now(), kind: SocketConsoleKind.incoming, text: text, source: 'client'))),
      _client.outgoing.listen((text) => _addConsole(SocketConsoleEntry(timestamp: DateTime.now(), kind: SocketConsoleKind.outgoing, text: text, source: 'client'))),
    ...
    return ModuleWorkbench(
      ...
      body: LayoutBuilder(
        builder: (context, constraints) {
          ...
          return narrow ? ListView(children: [main, const Divider(height: 1), composer]) : Row(children: [Expanded(child: main), const VerticalDivider(width: 1), Expanded(child: composer)]);
        },
      ),
  ...
  Widget _header() {
    ...
    return SingleChildScrollView(
      ...
      child: Row(
        children: [
          SegmentedButton<int>(
            segments: const [ButtonSegment(value: 0, label: Text('Server'), icon: Icon(Icons.dns)), ButtonSegment(value: 1, label: Text('Client'), icon: Icon(Icons.link))],
            selected: {_tab},
          ...
          if (_tab == 0) ...[
            SizedBox(width: 180, child: TextField(controller: _serverHost, decoration: const InputDecoration(labelText: 'Bind Host', isDense: true))),
            const SizedBox(width: 8),
            SizedBox(width: 96, child: TextField(controller: _serverPort, decoration: const InputDecoration(labelText: 'Port', isDense: true))),
            const SizedBox(width: 12),
            FilledButton.icon(onPressed: serverRunning ? _server.stop : _startServer, icon: Icon(serverRunning ? Icons.stop : Icons.play_arrow, size: 18), label: Text(serverRunning ? 'Stop' : 'Start')),
            const SizedBox(width: 8),
            ...
            const SizedBox(width: 8),
            SizedBox(width: 96, child: TextField(controller: _remotePort, decoration: const InputDecoration(labelText: 'Port', isDense: true))),
            const SizedBox(width: 12),
            FilledButton.icon(onPressed: clientConnected ? _client.disconnect : _connectClient, icon: Icon(clientConnected ? Icons.link_off : Icons.link, size: 18), label: Text(clientConnected ? 'Disconnect' : 'Connect')),
            const SizedBox(width: 8),
            OutlinedButton.icon(onPressed: _connectClient, icon: const Icon(Icons.refresh, size: 18), label: const Text('Reconnect')),
          ],
          const SizedBox(width: 12),
          Chip(
            avatar: Icon(_tab == 0 ? (serverRunning ? Icons.check_circle : Icons.stop_circle) : (clientConnected ? Icons.check_circle : Icons.cancel), size: 16, color: _tab == 0 ? (serverRunning ? Colors.green : Colors.red) : (clientConnected ? Colors.green : Colors.red)),
            label: Text(_tab == 0 ? _serverState.name : _clientState.name, style: const TextStyle(fontSize: 12)),
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Text('TCP Server', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w600)),
            const SizedBox(height: 16),
            Chip(label: Text(running ? 'Listening on ${_serverHost.text}:${_serverPort.text}' : 'Server stopped')),
            if (_clients.isNotEmpty) ...[
              const SizedBox(height: 16),
              Text('Connected Clients (${_clients.length})', style: Theme.of(context).textTheme.titleSmall),
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          mainAxisSize: MainAxisSize.min,
          children: [Text('TCP Client', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w600)), const SizedBox(height: 16), Chip(label: Text(connected ? 'Connected to ${_remoteHost.text}:${_remotePort.text}' : 'Client disconnected'))],
        ),
      ),
    );
  }
  ...
  Future<void> _startServer() async {
    try {
      await _server.start(host: _serverHost.text.trim(), port: int.tryParse(_serverPort.text) ?? 9000);
    } catch (error) {
      _addError(error);
    }
  }
  ...
  Future<void> _restartServer() async {
    try {
      await _server.restart(host: _serverHost.text.trim(), port: int.tryParse(_serverPort.text) ?? 9000);
    } catch (error) {
      _addError(error);
    }
  }
  ...
  Future<void> _connectClient() async {
    try {
      await _client.connect(_remoteHost.text.trim(), int.tryParse(_remotePort.text) ?? 9000);
    } catch (error) {
      _addError(error);
    }
  }
