...
class _SocketServerPageState extends State<SocketServerPage> {
  ...
  Future<void> _startServer() async {
    try {
      ...
      setState(() {
        ...
        _serverStatus = "Server đang chạy tại localhost:1234";
      });
  ...
  void _sendCardData(String cardId, String readerIndex) {
    ...
    final cardData = {
      "data": {
        "cardInfo": {
          "cardId": cardId,
          "readerIndex": int.tryParse(readerIndex) ?? 1,
          "readerName": "Reader ${int.tryParse(readerIndex) ?? 1}",
          "time": timeString,
        },
        "deviceInfo": _deviceInfo?.toJson() ?? {},
        "id": _deviceInfo?.deviceId ?? "",
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(SnackBar(content: Text('Đã gửi iOStatus: $cardId - $readerIndex')));
  }
  ...
  void _sendIoStatus(String inputName) {
    ...
    final List<Map<String, dynamic>> inputs =
        [
          ...
          return {
            ...
            "value": (item["inputName"] == inputName) ? 1 : 0,
          };
        }).toList();
    ...
    final relays = List.generate(8, (i) {
      return {
        "relayIndex": i + 1,
        "relayName": i < 4 ? "Lock ${i + 1}" : "AuxOut ${i - 3}",
        "value": 0,
      };
    });
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(SnackBar(content: Text('Đã gửi iOStatus: $inputName')));
  }
          ...
          IconButton(
            ...
            onPressed: () async {
              final result = await Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (_) => DeviceConfigPage(deviceInfo: _deviceInfo),
                ),
              );
      ...
      body: Column(
        children: [
          ...
          Container(
            ...
            color:
                _isServerRunning ? Colors.green.shade100 : Colors.red.shade100,
            child: Row(
              children: [
                Icon(
                ...
                Expanded(
                  child: Text(
                    'Dữ liệu lưu tại : ${_dataFile?.path}',
                    style: TextStyle(
                      fontWeight: FontWeight.bold,
                      color:
                          _isServerRunning
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(
                      color: _clients.isNotEmpty ? Colors.blue : Colors.grey,
                    ),
                  ),
                  child: Row(
          ...
          Expanded(
            child: ListView.builder(
              ...
              itemBuilder: (context, index) => _buildCardReaderRow(index),
            ),
          ),
          // Add/Remove buttons
          Container(
                            ...
                            content: Column(
                              ...
                              children: [
                                ListTile(
                                  ...
                                  onTap:
                                      () =>
                                          Navigator.pop(context, RowType.card),
                                ),
                                ListTile(
                                  leading: Icon(Icons.sensors),
                                  title: Text("Vòng từ (iOStatus)"),
                                  onTap:
                                      () => Navigator.pop(context, RowType.io),
                                ),
                              ],
                            ),
                          ),
