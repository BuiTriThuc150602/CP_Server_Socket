...
class TcpSocketMessage {
  const TcpSocketMessage({required this.sessionId, required this.text, required this.timestamp, required this.direction, this.bytes});
...
class TcpServerEngine {
  ...
  Future<void> start({required String host, required int port, bool shared = false}) async {
  ...
  Future<void> sendBytesToSessions(Iterable<String> sessionIds, List<int> bytes) async {
    for (final sessionId in sessionIds) {
      ...
      try {
        ...
        final message = TcpSocketMessage(sessionId: sessionId, text: preview, timestamp: DateTime.now(), direction: TcpMessageDirection.outgoing, bytes: List<int>.unmodifiable(bytes));
  ...
  void _handleClient(Socket socket) {
    ...
    _clientSubscriptions[id] = socket.listen(
      (bytes) {
        ...
        _emitClients();
        final message = TcpSocketMessage(sessionId: id, text: text, timestamp: DateTime.now(), direction: TcpMessageDirection.incoming, bytes: List<int>.unmodifiable(bytes));
  ...
  String _friendlyBindError(String host, int port, Object error) {
    if (error is SocketException) {
      ...
      if (message.contains('address already in use') || message.contains('only one usage') || error.osError?.errorCode == 10048 || error.osError?.errorCode == 98) {
        return 'Address already in use: $host:$port. Stop the other server or choose another port.';
      }
      if (message.contains('permission') || message.contains('access is denied') || error.osError?.errorCode == 10013 || error.osError?.errorCode == 13) {
        return 'Permission denied while binding $host:$port. Try another port or run with suitable permissions.';
      }
      if (message.contains('failed host lookup') || message.contains('address not available') || error.osError?.errorCode == 10049 || error.osError?.errorCode == 99) {
